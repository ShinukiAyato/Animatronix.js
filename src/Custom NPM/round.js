    ...
    constructor(opts) {
        ...
        if (opts.fitnesses && opts.fitnesses.constructor === Array) {
            for (var i = 0; i < opts.fitnesses.length; i++) {
                if (typeof opts.fitnesses[i].name !== 'undefined' && typeof opts.fitnesses[i].val !== 'undefined') {
                    //copy val instead of reference
                    this.fitnesses.push({
                        name: opts.fitnesses[i].name,
                        type: opts.fitnesses[i].type,
                        half: opts.fitnesses[i].half,
                        row: opts.fitnesses[i].row,
                        eod: opts.fitnesses[i].eod,
